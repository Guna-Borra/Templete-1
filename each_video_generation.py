...
def DownloadImages(result_links):
    global detection_length
    global desc_video_number
...
def GenerateSpeech(text):
    ...
    speech_gtts = "speech_gtts"+"_"+str(desc_video_number)+".wav"
    ...
    Trimmed_background = "Trimmed_background"+"_"+str(desc_video_number)+".wav"
...
def VideoGenerate(title, halfway_point, list_of_elements_new):
    ###########
    #Step 4 : 
    ###########
    ...
    command='''ffmpeg '''
    image_length=0
    output_video = "output_video"+"_"+str(desc_video_number)+".mp4"
    
    list_of_elements_new = [*list_of_elements_new]
    ...
    x=(((math.ceil(halfway_point/1000)))/(image_length))
    
    print('Video number & image length , time X', desc_video_number, image_length, x)
    ...
    for a in range(1,image_length):
        command+='''[{0}:v]format=pix_fmts=yuva420p,crop=w=2*floor(iw/2):h=2*floor(ih/2),scale=-6:6*ih,zoompan=z='if(eq(on,1),1,zoom+0.000417)':x='0':y='0':d=30*{2}:s=1280x2272,fade=t=in:st=0:d=1:alpha=1,fade=t=out:st={1}:d=1:alpha=1,setpts=PTS-STARTPTS+{1}/TB[v{0}];'''.format(a,a*x,floor(x))
...
def Video_Processing(result_links,title,text):
    ...
    if len(result_links) == 0:
        return "NO_IMAGE_LINKS"
    # 1 -> -----------------------------
    DownloadImages(result_links)
    # 2 -> -----------------------------
    halfway_point, Trimmed_background, speech_gtts, Final_Audio = GenerateSpeech(text)
    # 3 -> -----------------------------
    print('check-1')
    list_of_elements, list_of_elements_new = ProcessImages()
    # 4 -> -----------------------------
    output_video = VideoGenerate(title,halfway_point, list_of_elements_new)
    ...
    video_Final = "Output"+"_"+str(desc_video_number)+".mp4"
    ...
    video_name = "output"+"_" +"Final"+"_"+ str(desc_video_number)+".mp4"
    command_new ='''ffmpeg '''+"-i"+" "+ video_Final +" "+"-i"+" "+Final_Audio+" "+"-shortest"+" "+ video_name 
    os.system(command_new)
    str(desc_video_number)+".mp4"
    ###########
    #Step 7 : 
    ###########
    #Compress the final audio file that was generated in the above step
    video_News = "Output"+"_" +"prevideo"+"_"+ str(desc_video_number)+".mp4"    
    command_news ='''ffmpeg '''+"-i"+" "+video_name+" "+"-vcodec libx264 -crf 30"+" "+video_News
    os.system(command_news)
    desc_video_number = desc_video_number + 1 
    #title_lable=title_lable[:90] +".mp4"
    #os.rename(video_News,title_lable)
    ########
    #step 9:
    ########
    #add logo to generated video
    dir = '.'
    list_of_element = os.listdir(dir)
    list_of_element = natsort.natsorted(list_of_elements)
    for item in list_of_element:
        if item.endswith('.png') and item=='logo.png':
            video_with_logo = "Output"+"_" +"Video"+"_"+ str(desc_video_number)+".mp4" 
            command_news ='''ffmpeg '''+"-y"+" "+"-i"+" "+video_News+" "+"-i"+" "+item+" "+'''\-filter_complex "overlay=x=main_w*0.01:y=main_h*0.01"'''+" "+video_with_logo
            #ffmpeg -y -i Output_News_0.mp4 -i way2news_logo1.png \-filter_complex "overlay=x=main_w*0.01:y=main_h*0.01" output_with_logo.mp4
    ###########
    #Step 8: 
    ###########
